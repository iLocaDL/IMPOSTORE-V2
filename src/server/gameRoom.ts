...
export default class GameRoom {
  ...
  connect(connection: RoomConnection) {
    this.connections.set(connection.id, connection)
    console.log('[room] connection opened', this.roomId, connection.id)
    connection.send(
      { type: 'connected', playerId: connection.id, roomId: this.roomId } satisfies ServerMessage,
    )
  }

  handleMessage(message: string | ClientMessage, sender: RoomConnection) {
  ...
  disconnect(connection: RoomConnection) {
    if (this.connections.get(connection.id) !== connection) {
      console.info('[room] ignored stale connection close', this.roomId, connection.id)
      return
    }
  ...
  private createRoom(connection: RoomConnection, name: string, mode: GameMode) {
    ...
    connection.send(
      { type: 'roomCreated', playerId: connection.id, roomId: this.roomId, state } satisfies ServerMessage,
    )
    console.log('[room] initialized and sent roomCreated', this.roomId)
    this.broadcastRoomState()
  }

  private joinRoom(connection: RoomConnection, name: string) {
    if (!name.trim()) {
      this.sendError(connection, 'Nome non valido.')
      return
    }
    ...
    if (this.phase !== 'lobby' && this.phase !== 'setup') {
  ...
  private closeRoom(leavingHostId: string) {
    ...
    for (const [connectionId, connection] of this.connections) {
      if (connectionId !== leavingHostId) {
        connection.send({ type: 'roomClosed', message } satisfies ServerMessage)
      }
    }
  ...
  private startSetup(sender: RoomConnection) {
  ...
  private submitAnswer(sender: RoomConnection, answer: string) {
  ...
  private showAnswers(sender: RoomConnection) {
  ...
  private showResults(sender: RoomConnection) {
  ...
  private startNewGame(sender: RoomConnection) {
  ...
  private sendChatMessage(sender: RoomConnection, toPlayerId: string, text: string) {
  ...
  private markChatAsRead(sender: RoomConnection, withPlayerId: string) {
  ...
  private broadcastRoomState() {
    for (const [connectionId, connection] of this.connections) {
      connection.send(
        { type: 'roomState', state: this.getPublicStateForConnection(connectionId) } satisfies ServerMessage,
      )
    }
  }
  ...
  private sendPersonalQuestions() {
    ...
    for (const assignment of this.game.assignments) {
      this.connections
        ...
        ?.send({ type: 'yourQuestion', question: assignment.question } satisfies ServerMessage)
    }
  }
  ...
  private sendError(connection: RoomConnection, message: string) {
    connection.send({ type: 'error', message } satisfies ServerMessage)
  }
}
